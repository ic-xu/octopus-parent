...
public class UdpSenderPool {
    ...
    public UdpSenderPool(TransportConfig config) throws IOException {
        this.config = config;
...
//                    DatagramPacket receiveDatagramPacket = new DatagramPacket(new byte[11], 11);
                    //读缓冲
                    ...
                    while (true) {
                        trySendCount++;
                        if (selector.select(config
                                .getIntegerProperties(
                                        UdpTransportConstants
                                                .udpTransportSegmentSendTimeOut,
                                        3000)) < 1) {
                            if (trySendCount > config.getIntegerProperties(
                                    UdpTransportConstants.udpTransportSegmentSendTimeOutCount,3)) {
                                throw new RuntimeException(messageWrapper +" send time out");
                            }
                            channel.send(buffer, messageWrapper.getDescInetAddress());
                            continue;
                        }
                        ...
                        while (it.hasNext()) {
                            ...
                            if (nextKey.isReadable()) {
                                ...
                                if (readBuffer.get(0) == 1) {
                                    if (readBuffer.getChar(1)
                                            == i && readBuffer.getLong(3)
                                            == messageId) {
                                        break exit;
                                    }
                                }
