...
//  private def getMQTTConnection2Channel(channel: Channel) = channel.attr(ATTR_KEY_CONNECTION).get.asInstanceOf[MQTTConnection]
  ...
  override def channelRead(ctx: ChannelHandlerContext, message: Object): Unit = {
    val msg = NettyUtils.validateMessage(message)
    //从channel 中获取相应的连接对象，通过连接对象处理相应的事务
    val mqttConnection: MQTTConnection = NettyUtils.getMQTTConnection2Channel(ctx.channel)
    try mqttConnection.handleMessage(msg)
    catch {
      case ex: Throwable =>
        //ctx.fireExceptionCaught(ex);
        logger.error("Error processing protocol message: {}", msg.fixedHeader.messageType, ex)
        ctx.channel().close().addListener((_: ChannelFutureListener) => logger.info("Closed client channel due to exception in processing")
      )
