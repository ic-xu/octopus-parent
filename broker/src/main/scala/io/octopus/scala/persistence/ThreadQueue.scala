...
import io.octopus.base.queue.{MsgQueue, SearchData, StoreMsg}
...
class ThreadQueue(config: IConfig, mvStore: MVStore, checkPointServer: CheckPointServer, flushDiskServiceExecutor: ScheduledExecutorService)
  ...
  override def initialize(params: Any): Unit = {
...
//      .asInstanceOf[MsgDispatcher]
   ...
   */
  override def offer(msg: IMessage): StoreMsg[IMessage] = {
   ...
   */
  override def poll(): StoreMsg[IMessage] = getConcurrentFileQueue.poll()
   ...
   * @return E
   */
  override def poll(searchData: SearchData): StoreMsg[IMessage] = {
