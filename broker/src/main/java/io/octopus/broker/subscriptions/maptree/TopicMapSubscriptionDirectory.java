...
import io.octopus.broker.subscriptions.plugin.imp.DefaultTopics;
import io.octopus.broker.subscriptions.plugin.imp.IntersectionTopics;
...
public class TopicMapSubscriptionDirectory implements ISubscriptionsDirectory {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(TopicMapSubscriptionDirectory.class);
...
//    }
    ...
    public Set<Subscription> matchQosSharpening(Topic topic, boolean isNeedBroadcasting) {
        ...
        if (isNeedBroadcasting) {
            try {
//                Set<Subscription> sys_broadcasting = rootTopicRegisterMap.getChildren().get(ConstantsTopics.$SYS_BROADCASTING).getSubscriptions();
                Set<Subscription> sys_broadcasting = subscriptionsRepository.retrieveSub(ConstantsTopics.$SYS_BROADCASTING);
