...
public class PostOffice {
    ...
    PostOffice(ISubscriptionsDirectory subscriptions, IRetainedRepository retainedRepository,
               SessionRegistry sessionRegistry, BrokerNotifyInterceptor interceptor, Authorizator authorizator) {
    ...
    public void fireWill(Session.Will will, MqttConnection connection) {
        // MQTT 3.1.2.8-17
        Topic topic = new Topic(will.topic);
        publish2Subscribers(will.payload, topic, will.qos, true);

        //retained message
        if (will.retained) {
            if (!will.payload.isReadable()) {
                retainedRepository.cleanRetained(topic);
            } else {
                // before wasn't stored
                MqttFixedHeader fixedHeader = new MqttFixedHeader(MqttMessageType.PUBLISH, false, will.qos, true, 0);
                MqttPublishVariableHeader varHeader = new MqttPublishVariableHeader(will.topic,connection.nextPacketId());
                MqttPublishMessage mqttPublishMessage = new MqttPublishMessage(fixedHeader, varHeader, will.payload);
    ...
    private void publishRetainedMessagesForSubscriptions(String clientID, List<Subscription> newSubscriptions) {
        ...
        for (Subscription subscription : newSubscriptions) {
            ...
            for (RetainedMessage retainedMsg : retainedMsgs) {
                ...
                targetSession.sendPublishOnSessionAtQos(retainedMsg.getTopic(), qos, payloadBuf);
//                targetSession.sendRetainedPublishOnSessionAtQos(retainedMsg.getTopic(), qos, payloadBuf);
    ...
    public void cleanSubscribe(List<Subscription> subs){
    ...
    public void unsubscribe(List<String> topics, MqttConnection mqttConnection, int messageId) {
        ...
        for (String t : topics) {
            ...
            if (!validTopic) {
                ...
                LOGGER.warn("Topic filter is not valid. CId={}, topics: {}, offending topic filter: {}", clientID,
                        topics, topic);
                return;
            }
...
//            clientSession.unsubscribeFrom(topic);
    ...
    void receivedPublishQos0(Topic topic, String username, String clientID,
        ...
        //TODO 判断账号是用户的还是转发系统的，这个是针对之前router 服务转发的，现在该用UDP直接发送
//        boolean isNeedBroadcasting = (registerUserName != null && !registerUserName.contains(username));
        publish2Subscribers(msg.payload(), topic, AT_MOST_ONCE, false);
    ...
    void receivedPublishQos1(MqttConnection connection, Topic topic, String username, int messageID, MqttPublishMessage msg) {
        ...
        boolean isNeedBroadcasting = (registerUserName != null && !registerUserName.contains(username));
        publish2Subscribers(msg.payload(), topic, AT_LEAST_ONCE, isNeedBroadcasting);
    ...
    void receivedPublishQos2(MqttConnection connection, MqttPublishMessage mqttPublishMessage, String username) {
        LOGGER.trace("Processing PUBREL message on connection: {}", connection);
