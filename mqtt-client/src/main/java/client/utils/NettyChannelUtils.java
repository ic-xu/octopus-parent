...
public class NettyChannelUtils {
    ...
    private static final AttributeKey<Object> ATTR_KEY_CONNECTION_OpTION = AttributeKey.valueOf(ATTR_KEY_CONNECT_OPTION);

    public static void setConnectOption(Channel channel, MqttConnectOptions options) {
        channel.attr(ATTR_KEY_CONNECTION_OpTION).set(options);
    }

    public static MqttConnectOptions getConnectOption(Channel channel){
        return (MqttConnectOptions) channel.attr(ATTR_KEY_CONNECTION_OpTION).get();
    }
