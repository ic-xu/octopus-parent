...
//                    datagramPacket1.setData(buffer.array(),0,buffer.array().length);
                    ...
                    while (true) {
                        ...
                        trySendCount++;
                        if (selector.select(config.getIntegerProperties(UdpTransportConstants.UDP_TRANSPORT_SEGMENT_SEND_TIME_OUT, 20)) < 1) {
                            if (trySendCount > config.getIntegerProperties(UdpTransportConstants.UDP_TRANSPORT_SEGMENT_SEND_TIME_OUT_COUNT, 3)) {
