...
if exist "%OCTOPUS_HOME%\bin\octopus.bat" goto okHome
cd ..
set "OCTOPUS_HOME=%cd%"
cd "%CURRENT_DIR%"
:gotHome
if exist "%OCTOPUS_HOME%\bin\octopus.bat" goto okHome
    echo TheOCTOPUS_HOME environment variable is not defined correctly
...
echo Using OCTOPUS_HOME:   "%OCTOPUS_HOME%"
...
set OCTOPUS_PATH=%OCTOPUS_HOME%
set LOG_FILE=%OCTOPUS_HOME%\config\octopus-log.properties
...
rem set JAVA_OPTS=%JAVA_OPTS% -Xloggc:/var/log/octopus/gc.log
...
%JAVA% -server %JAVA_OPTS% %JAVA_OPTS_SCRIPT% -Dlog4j.configuration=file:%LOG_FILE% -Doctopus.path=%OCTOPUS_PATH% -cp %OCTOPUS_HOME%\lib\* io.octopus.broker.Server
